...
from .models import (
    ...
    DataBreach, Task, CrossBorderTransfer, Report, ReportSchedule,
    EncryptedField
)
from .forms import (
        ...
        if form.is_valid():
            try:
                with transaction.atomic():
                    ...
                    # Get the next URL from query parameters or use default
                    next_url = request.GET.get('next')
                    if next_url and is_safe_url(next_url, allowed_hosts=django_settings.ALLOWED_HOSTS):
...
def export_user_data(request):
    ...
    try:
        ...
        return render(request, 'user_templates/data_export.html', {
...
def security_dashboard(request):
    ...
    try:
        ...
        security_logs = AuditLog.objects.filter(
            user=request.user,
            action__in=['login', 'logout', '2fa_enabled', '2fa_disabled', 'password_changed']
        ...
        return render(request, 'gdpr/security_dashboard.html', context)
        
    except Exception as e:
        logger.error(f"Security dashboard error for user {request.user.id}: {str(e)}")
...
def trusted_devices(request):
    ...
    try:
        ...
        # Get trusted devices
        trusted_devices = UserSession.objects.filter(
            user=request.user,
            is_trusted=True,
            is_active=True
        ).order_by('-login_time')
        ...
        logger.error(f"Trusted devices view error for user {request.user.id}: {str(e)}")
    ...
    try:
        if request.method == 'POST':
            ...
            user_data = {
                ...
                'privacy_settings': request.user.privacy_settings if hasattr(request.user, 'privacy_settings') else {},
                'marketing_preferences': request.user.marketing_preferences if hasattr(request.user, 'marketing_preferences') else {},
                'data_retention_policy': request.user.data_retention_policy if hasattr(request.user, 'data_retention_policy') else {}
            }
            ...
            response['Content-Disposition'] = f'attachment; filename="data_export_{request.user.username}_{timezone.now().strftime("%Y%m%d")}.csv"'
            ...
            AuditLog.objects.create(
                ...
                resource_type='user_data',
                resource_id=str(request.user.id),
                details={'request_id': str(export_request.id)}
            )
        ...
        logger.error(f"Data export error for user {request.user.id}: {str(e)}")
        messages.error(request, _('An error occurred during data export.'))
        return redirect('gdpr_platform:dashboard')
...
def handle_settings_update(request):
    ...
    try:
        ...
        settings.platform_name = request.POST.get('platform_name')
        settings.support_email = request.POST.get('support_email')
        settings.default_language = request.POST.get('default_language')
        settings.timezone = request.POST.get('timezone')
        
        # Update security settings
        settings.enforce_2fa = request.POST.get('enforce_2fa') == 'true'
        settings.session_timeout = int(request.POST.get('session_timeout'))
...
def user_dashboard(request):
    ...
    try:
        ...
        data_requests = DataRequest.objects.filter(user=request.user).order_by('-request_date')[:5]
        
        # Get latest cookie consent
        cookie_consent = CookieConsent.objects.filter(
            user=request.user
        ).order_by('-timestamp').first()
        
        # Get latest privacy policy consent
        privacy_consent = UserPrivacyPolicyConsent.objects.filter(
            user=request.user
        ).order_by('-consent_date').first()
        
        # Get unacknowledged breach notifications
        breach_notifications = BreachNotification.objects.filter(
            user=request.user,
        ...
        logger.error(f"Error in user dashboard for {request.user.id}: {str(e)}")
        messages.error(request, _('An error occurred while loading your dashboard. Please try again later.'))
        return redirect('gdpr_platform:landing')
...
def dashboard(request):
    ...
    try:
        # Ensure DPO permissions are up to date
